...
import ec.com.uce.jano.web.beans.DetalleCatalogoBean;
...
public class DetalleCatalogoBacking implements Serializable {
	...
	public List<SelectItem> getCatalogosItems() throws HiperionException {
		...
		if (catalogos != null) {
			for (Catalogo catalogo : catalogos) {
				SelectItem selectItem = new SelectItem(catalogo.getIdCatalogo(), catalogo.getDescripcionCatalogo());
	...
	public void selectCatalogo() throws HiperionException {
		Long codigoCatalogo = detalleCatalogoBean.getCodigoCatalogo();
		...
		if (detallesCatalogo != null) {
			...
			setActivarDetCatalogoPanel(true);
			detalleCatalogoBean.setDescripcionCatalogo(catalogo.getDescripcionCatalogo());
		}
	}
	...
	public void guardarDetalleCatalogo(ActionEvent event) throws HiperionException {
		...
		detalleCatalogo.setDescDetCatalogo(detalleCatalogoBean.getDescripcionCatalogo());
	...
	public void onEdit(RowEditEvent event) throws HiperionException {
		...
		FacesMessage msg = new FacesMessage("Detalle Catalogo", detCatalogo.getDescDetCatalogo());
		FacesContext.getCurrentInstance().addMessage(null, msg);
	}

	public void onCancel(RowEditEvent event) {
		FacesMessage msg = new FacesMessage("Detalle Catalogo Cancelado", ((DetalleCatalogo) event.getObject()).getDescDetCatalogo());
		FacesContext.getCurrentInstance().addMessage(null, msg);
	}
